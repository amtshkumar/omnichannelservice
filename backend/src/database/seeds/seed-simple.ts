...
async function seed() {
  ...
  try {
    ...
    const hashedPassword = '$2b$10$rZ8qN5YYXxH5h5h5h5h5hOe5YYXxH5h5h5h5hOqN5YYXxH5h5h5h';
    ...
    // Email provider config (Mock for development) - plain JSON, no encryption for dev
    await dataSource.query(
      ...
      [
        tenantId,
        JSON.stringify({ apiKey: 'mock-sendgrid-key' }),
        JSON.stringify({
    ...
    await dataSource.query(
      ...
      [
        tenantId,
        JSON.stringify({ accountSid: 'mock-twilio-sid', authToken: 'mock-twilio-token' }),
        JSON.stringify({ fromNumber: '+1234567890' }),
      ],
    );
